...
stopifnot(!is.null(params$statcheckOutput))
stat <- params$statcheckOutput
```

```{r}
stat$Consistency[stat$error == FALSE & stat$decision_error == FALSE] <- "Consistent"
stat$Consistency[stat$error == TRUE & stat$decision_error == FALSE] <- "**Inconsistency**"
stat$Consistency[stat$decision_error == TRUE] <- "**Decision Inconsistency**"
...
stat_sparse <- cbind(stat_sparse, Code)
...
statcheck (Nuijten & Epskamp, `r year`) is an R package that automatically extracts statistical results from papers and checks the internal consistency of those results.
...
Please note that statcheck is an automated procedure and does not offer any explanations for detected inconsistencies (e.g., incorrect rounding, erroneous retrieval from computer output, a copy-paste error, or a typo).
...
For more details on what statcheck can and cannot do, and a list of common reasons why statcheck either does not find statistics or flags them as inconsistent, see the FAQ at <https://statcheck.io>. 
