...
test_that("simple inconsistencies are classified as such", {
  ...
  expect_true(statcheck(txt1)$Error)
  expect_true(statcheck(txt2)$Error)
  expect_true(statcheck(txt3)$Error)
  expect_true(statcheck(txt4)$Error)
  expect_true(statcheck(txt5)$Error)
  expect_true(statcheck(txt6)$Error)
})
...
test_that("correctly rounded p-values are not considered errors", {
  ...
  expect_false(statcheck(txt1)$Error)
  expect_false(statcheck(txt2)$Error)
  expect_false(statcheck(txt3)$Error)
  expect_false(statcheck(txt4)$Error)
  expect_false(statcheck(txt5)$Error)
  expect_false(statcheck(txt6)$Error)
})
...
test_that("OneTailedTests considers everything as one-tailed", {
  ...
  expect_false(statcheck(txt1, OneTailedTests = TRUE)$Error)  
  expect_true(statcheck(txt2, OneTailedTests = TRUE)$Error)
  expect_equal(statcheck(txt3, OneTailedTests = TRUE)$Error, c(FALSE, TRUE))
})
...
test_that("automated one-tailed test detection works", {
  ...
  expect_true(statcheck(txt1)$Error)
  expect_true(statcheck(txt1, OneTailedTxt = TRUE)$Error)
  expect_true(statcheck(txt2, OneTailedTxt = TRUE)$Error)
  expect_true(statcheck(txt3)$Error)
  
  # correct for one-tailed testing here
  expect_false(statcheck(txt3, OneTailedTxt = TRUE)$Error)
  expect_false(statcheck(txt4, OneTailedTxt = TRUE)$Error)
  expect_false(statcheck(txt5, OneTailedTxt = TRUE)$Error)
})
...
test_that("you can adapt whether p = .000 is counted as inconsistent or not", {
  ...
  expect_true(statcheck(txt1)$Error)
  expect_false(statcheck(txt1, pZeroError = FALSE)$Error)
  
  expect_true(statcheck(txt2)$Error)
  expect_true(statcheck(txt2, pZeroError = FALSE)$Error)
})
