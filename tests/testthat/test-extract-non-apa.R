...
# check if APA results are still parsed correctly when apa_style = FALSE
test_that("simple errors are classified as such, also when apa_style = FALSE", {
  txt1 <- "t(28) = 2.20, p = .03"
  ...
  expect_true(statcheck(txt1, apa_style = FALSE, messages = FALSE)[[VAR_ERROR]])
  expect_true(statcheck(txt2, apa_style = FALSE, messages = FALSE)[[VAR_ERROR]])
  expect_true(statcheck(txt3, apa_style = FALSE, messages = FALSE)[[VAR_ERROR]])
  expect_true(statcheck(txt4, apa_style = FALSE, messages = FALSE)[[VAR_ERROR]])
  expect_true(statcheck(txt5, apa_style = FALSE, messages = FALSE)[[VAR_ERROR]])
  expect_true(statcheck(txt6, apa_style = FALSE, messages = FALSE)[[VAR_ERROR]])
})
...
test_that("DF in degrees of freedom is extracted and parsed", {
  ...
  txt3 <- "Q(df=781)=9,481.65,p<.001"
  
  result <- statcheck(c(txt1, txt2, txt3), apa_style = FALSE, 
                      messages = FALSE)
  
  expect_equal(nrow(result), 3)
  expect_equal(result[[VAR_DF1]], c(2, 3, 781))
